...
async fn list_tools_from_config(config: serde_json::Value) -> Result<Vec<mcp::MCPTool>, String> {
    ...
    println!("🚀 [TAURI] Config received: {}", serde_json::to_string_pretty(&config).unwrap_or_default());
    
    // config에서 servers 배열을 파싱
    let servers = config.get("servers")
        .and_then(|v| v.as_array())
        .ok_or("Invalid config: missing servers array")?;
    ...
    for (i, server_value) in servers.iter().enumerate() {
        ...
        println!("🚀 [TAURI] Server: {} (command: {:?}, transport: {})", 
        ...
        match manager.list_tools(&server_name).await {
            Ok(tools) => {
                println!("✅ [TAURI] Got {} tools from server {}", tools.len(), server_name);
                all_tools.extend(tools);
            },
            Err(e) => {
                eprintln!("❌ [TAURI] Error getting tools for server {}: {}", server_name, e);
            }
        }
    }
